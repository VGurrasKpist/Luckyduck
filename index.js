...
function translateEnglish(){
  ...
  data.forEach(function(currentValue,index){
    ...
    // Shopcart start
    function cartDeleteEventListener(item) {
    ...
    function cartBuyEventListener(item) {
      const cartItem =cartItems.find((cartItem) => cartItem.item.id ===item.id);
      if (cartItem) {
        cartItem.quantity++;
      } else {
...
spans.forEach(span => {
    ...
    function updateCart() {
      ...
      const cartContent =cartItems
    .map((cartItem) => `${cartItem.item.title.sv} - ${Array.isArray(cartItem.item.price) ? cartItem.item.price[0] : cartItem.item.price}kr (Antal: ${cartItem.quantity}, Summa: ${Array.isArray(cartItem.item.price) ? cartItem.item.price[0] * cartItem.quantity : cartItem.item.price * cartItem.quantity} kr)`)
    .join("<br>");
